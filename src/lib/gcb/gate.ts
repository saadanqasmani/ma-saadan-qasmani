 ...
 * The gate on GCB.
 *
 * Everything under /gcb is private work. The middleware turns away any
...
export const GCB_COOKIE = "gcb_access";

/** A month. Long enough that the code is asked for rarely. */
export const GCB_COOKIE_MAX_AGE = 60 * 60 * 24 * 30;

/**
 * Set GCB_CODE in the host's environment to rotate it without a deploy.
 * The fallback is the code in use today.
 */
export function gcbCode(): string {
  return (process.env.GCB_CODE || "taraedu12345").trim();
}
...
export function gcbToken(): Promise<string> {
  return sha256Hex(`gcb/v1/${gcbCode()}`);
}
...
export function gcbCodeIsCorrect(entered: string): boolean {
  return sameSecret(entered.trim().toLowerCase(), gcbCode().toLowerCase());
}

export async function gcbTokenIsValid(value: string | undefined): Promise<boolean> {
  if (!value) return false;
  return sameSecret(value, await gcbToken());
}
