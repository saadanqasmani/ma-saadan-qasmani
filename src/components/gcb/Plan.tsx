...
import { documents, providerLabel, type Doc } from "@/content/gcb/documents";
import { usePoints, Star } from "@/components/gcb/Points";
import { serverSnapshot, snapshot, subscribe, write } from "@/lib/gcb/browserStore";

const STORE = "gcb-plan";
...
export function Plan() {
  ...
  return (
    <div>
      {/* Add a university */}
      <form onSubmit={addTarget} className="gcb-pane" style={{ padding: "1.5rem" }}>
        <label className="gcb-label" style={{ color: "var(--text-faint)" }}>
        ...
        <div style={{ marginTop: "0.9rem", display: "flex", flexWrap: "wrap", gap: "0.6rem" }}>
          ...
          />
          <button type="submit" className="gcb-btn gcb-btn--primary">Add</button>
        </div>
        <p className="gcb-small" style={{ marginTop: "0.85rem" }}>
      ...
      {state.targets.length === 0 && (
        <div className="gcb-pane gcb-land" style={{ marginTop: "1rem", padding: "2rem", textAlign: "center" }}>
          <p className="gcb-h2" style={{ fontSize: "1.125rem" }}>Nothing on your list yet.</p>
          <p className="gcb-body" style={{ marginTop: "0.6rem" }}>
      ...
      <div style={{ marginTop: "1rem", display: "grid", gap: "0.9rem" }}>
        {state.targets.map((t) => {
          ...
          return (
            <div key={t.id} className="gcb-pane" data-open={isOpen} style={{ padding: "1.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "1.1rem", flexWrap: "wrap" }}>
                ...
                >
                  <h3 className="gcb-h2" style={{ fontSize: "1.125rem" }}>{t.name}</h3>
                  <p className="gcb-small" style={{ marginTop: "0.25rem" }}>
              ...
              <div className="gcb-reveal" data-open={isOpen} style={{ marginTop: isOpen ? "1.5rem" : 0 }}>
                <div>
                  <DocList
                    target={t}
                    onToggle={toggleDoc}
                  ...
                  {ready && (
                    <div className="gcb-land" style={{ marginTop: "1.5rem", padding: "1.25rem", borderRadius: "12px", border: "1px solid var(--free)" }}>
                      <span className="gcb-label" style={{ color: "var(--free)" }}>Everything is in</span>
                      <p className="gcb-body" style={{ marginTop: "0.6rem" }}>
                      ...
                      <div style={{ marginTop: "1rem", display: "flex", flexWrap: "wrap", gap: "0.6rem" }}>
                        <Link href="/gcb/apply" className="gcb-btn gcb-btn--ghost">Apply myself</Link>
                        <Link href="/gcb/pricing" className="gcb-btn gcb-btn--primary">Have it reviewed</Link>
                      </div>
                    </div>
                  )}
                </div>
  ...
  return (
    <div style={{ display: "grid", gap: "1.5rem" }}>
      {byProvider.map(([provider, docs]) => (
        <div key={provider}>
          <p className="gcb-label" style={{ color: provider === "school" ? "var(--paid)" : "var(--text-faint)" }}>
                ...
                >
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p className="gcb-body" style={{ color: "var(--text)", textDecoration: done ? "line-through" : "none", opacity: done ? 0.55 : 1 }}>
                        {d.name}
                        {d.optional && <span className="gcb-small"> · if asked for</span>}
                      </p>
                      <p className="gcb-small" style={{ marginTop: "0.3rem" }}>{d.what}</p>
                      <p className="gcb-small" style={{ marginTop: "0.35rem", color: "var(--text-soft)" }}>
                        {d.watch}
                      </p>
                      <div style={{ marginTop: "0.55rem", display: "flex", gap: "0.9rem", flexWrap: "wrap" }}>
                        <span className="gcb-small">Allow about {d.leadDays} days</span>
                        <button
                          type="button"
...
function StuckPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="gcb-pane gcb-land"
        onClick={(e) => e.stopPropagation()}
        style={{ padding: "1.85rem", width: "min(28rem, 100%)" }}
      >
        <span className="gcb-label" style={{ color: "var(--accent)" }}>Stuck</span>
        <h3 className="gcb-h2" style={{ marginTop: "0.85rem", fontSize: "1.25rem" }}>
          Try the free route first.
        </h3>
        <p className="gcb-body" style={{ marginTop: "0.75rem" }}>
        ...
        <div style={{ marginTop: "1.5rem", display: "flex", flexWrap: "wrap", gap: "0.6rem" }}>
          <button type="button" onClick={onClose} className="gcb-btn gcb-btn--ghost">
            I will try myself
          </button>
          <Link href="/gcb/pricing" className="gcb-btn gcb-btn--primary">See Plus</Link>
        </div>
      </div>
    </div>
  );
