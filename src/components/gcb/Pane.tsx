    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem" }}>
          <div>
            <h3 className="gcb-h2" style={{ fontSize: "1.125rem" }}>{title}</h3>
            <p className="gcb-body" style={{ marginTop: "0.5rem" }}>{summary}</p>
          </div>
          <span
            aria-hidden
            style={{
      ...
      <div className="gcb-reveal gcb-pane__lift" data-open={open} style={{ marginTop: open ? "1.25rem" : 0 }}>
        <div>
          {children}
          {!alreadyEarned && (
            <p className="gcb-small" style={{ marginTop: "1rem", display: "flex", alignItems: "center", gap: "0.4rem", color: "var(--free)" }}>
              <Star size={12} /> +{points}
            </p>
          )}
        </div>
