...
/**
 * One frame of the title sequence, drawn.
 *
 * Pure: it takes a frame and returns the picture of it, with no clock, no
 * state and no browser in it. That is what lets the same art run two ways —
 * as the overlay the site plays on first visit, and as a Remotion composition
 * rendered to video, where every frame can be inspected as a still rather
 * than chased in a live preview.
 */
  ...
  return (
    <g opacity={dim ? 0.75 : 1}>
      {PEOPLE.map((p, i) => {
        ...
        return (
          <g key={i} fill={depth > 0.5 ? C.crowd : C.crowdFar} opacity={0.86 + depth * 0.14}>
...
export function IntroScene({ frame }: { frame: Frame }) {
